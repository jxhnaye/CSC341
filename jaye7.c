...
void swap_values(int *value1, int *value2, int *value3, int *value4);
...
int main(void)
{
	...
	*/
	...
	//to see the address stored in the pointer we print out the pointer normally
	//as it only stores addresses.
    ...
    //display var1 and var2 before swapping
    printf("Before swapping: var1 = %d, var2 = %d\n", var1, var2);
    
    swap_values(&valueReturned, &expected, &var1, &var2);

    printf("After swapping: expected = %d, valueReturned = %d\n", expected, valueReturned);
